...
test.describe('TEST-01: Login flow', () => {
  test('shows login screen on first visit', async ({ page }) => {
    ...
    await expect(page.getByText('КТК · Тренажёрный комплекс')).toBeVisible()
    await expect(page.getByText('Установка ЭЛОУ-АВТ')).toBeVisible()
  })
  ...
  test('can switch role tabs', async ({ page }) => {
    ...
    await expect(instructorTab).toHaveClass(/roleTabActive/)
  })
