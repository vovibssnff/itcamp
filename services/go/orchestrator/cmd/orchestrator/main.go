...
func main() {
	...
	if cfg.Clients.UseMock {
		...
		log.Info("using real HTTP clients",
			"assessment", cfg.Clients.AssessmentURL,
			"snapshot", cfg.Clients.SnapshotURL,
			"sim", cfg.Clients.SimURL,
		)
		simClient = client.NewHTTPSimClient(cfg.Clients.SimURL)
